...
const App = () => {
  ...
  const [getMovie, setGetMovie] = useState<Array<TopViewTypes>>([]);
  ...
  useEffect(() => {
    if (searchValue) {
      const getMovie = allMovies.filter((movie) =>
        movie.movie_name
          .toLowerCase()
          .replace(/ /g, "")
      ...
      <BrowserRouter>
        ...
        <div className="flex justify-between w-full z-50 p-5 fixed top-0 right-0 left-0 bg-bodyBackgroundColor">
          ...
          <div className="flex items-center justify-around md:justify-between gap-1 w-5/6 md:gap-5 md:w-[70%] lg:w-[80%] lg:pr-10 ">
            ...
            {/* User Login */}
            <Link to={"/login"} onClick={() => dispath(changeActiveLink(""))}>
              <button className="p-2 rounded-full hover:bg-containerBackgroundColor">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
        ...
        <div className="md:flex-row flex gap-10 flex-col ">
          ...
          <div className={`md:w-[75%] md:pt-24`}>
            <Routes>
              ...
              <Route
                path="/search-movie"
                element={<SearchPage getMovies={getMovie} />}
              />
